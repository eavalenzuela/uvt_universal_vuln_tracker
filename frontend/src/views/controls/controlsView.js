...
function renderControlCard(control) {
  return el(
    ...
    el(
      "div",
      { class: "row flex-between items-baseline flex-wrap" },
      el("div", { class: "font-semibold", text: control.name || "Untitled control" }),
      renderFrameworkChip(control.framework),
    ),
    control.description
      ? el("p", { class: "muted mt-8", text: control.description })
      : el("p", { class: "muted", style: "margin: 8px 0 0;", text: "No description provided." }),
    el(
      "div",
      { class: "row gap-16 mt-12 flex-wrap" },
...
export async function ControlsView() {
  ...
  const list = el("div", { class: "flex-col-10 mt-12" });
  ...
  const createForm = el(
    ...
    el("div", { class: "row flex-end" }, createSubmit),
  );
  ...
  const page = el(
    ...
    el("div", { class: "row flex-between flex-wrap gap-8", style: "margin: 12px 0;" }, summary, writable ? createToggle : null),
