...
def map_pci_dss_record(payload: Mapping[str, Any]) -> NormalizedControl:
    control_id = _get_first_str(payload, "requirement", "control_id", "id", "number") or "Unknown"
    title = _get_first_str(payload, "title", "name") or control_id
    description = _get_first_str(payload, "description", "summary")
    version = _get_first_str(payload, "version", "pci_version", "release")
    source_url = _get_first_str(payload, "url", "source_url", "sourceUrl", "link")
    framework = _get_first_str(payload, "framework") or "PCI DSS"
